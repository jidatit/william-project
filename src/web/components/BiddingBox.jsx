...
const BiddingBox = ({ vehicleId, carData }) => {
	...
	const { setDaysLeft, setHighestBid, calculateDaysLeft, status, setBidslen } =
		useListing();

	useEffect(() => {
		const lastBid =
			vehicleBids.length > 0
				? vehicleBids[vehicleBids.length - 1].amount
				: carData.price;
	...
	const makeAbidforVehicleId = async () => {
		try {
			...
			const updatedBids = [...vehicleBids, newBid].sort(
				(a, b) => parseFloat(a.amount) - parseFloat(b.amount),
			);
	...
	const handleMakeBid = (bid) => {
		...
		setTimeout(() => {
			...
			if (bid > lastBid) {
				...
				toast.error(
					"Bid must be higher than the last bid or the car's price.",
					{
						autoClose: 1000,
					},
				);
	...
	return (
		<>
			{currentUserId === currentUserAd && (
				<p className="font-bold text-center">You cannot bid on your own ad </p>
			)}
			<div
				ref={containerRef}
				className="w-full p-2 flex flex-col justify-start items-center max-h-[371px] overflow-y-auto min-h-[370px] bg-white border border-gray-200 rounded-lg shadow-lg"
			>
				<ToastContainer autoClose={1000} />
				{vehicleBids.length > 0 ? (
					vehicleBids.map((bid, index) => <BidBadge key={index} bid={bid} />)
				) : (
					...
					{currentUserId !== currentUserAd && (
						<>
							<TextField
								...
								className="w-full md:w-[70%] outline-none focus:outline-none"
								placeholder="Type amount of your bid here!"
							/>
							<button
								onClick={() => handleMakeBid(parseFloat(bidAmount))}
								type="button"
								className="focus:outline-none text-white md:w-[30%] w-full md:h-full bg-[#ffe001] hover:bg-yellow-500 focus:ring-yellow-300 font-medium text-sm px-5 py-2.5"
							>
								{Loading ? (
									<BeatLoader
										color={"#000000"}
