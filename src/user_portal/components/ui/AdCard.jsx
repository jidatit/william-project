			...
			<div className="w-full relative flex flex-col justify-start mx-auto bg-white rounded-xl shadow-md border-2 border-gray-200">
				<div className="w-full flex flex-col lg:flex-row justify-start items-start gap-5 p-3">
					<div className="w-full lg:w-fit h-full bg-white ">
						<img
							src={
								data.images.length > 0 ? data.images[0].file : placeholderImage
							}
							alt="tailwind logo"
							className="rounded-xl w-full lg:min-w-[260px] h-auto lg:h-[180px] cursor-pointer"
						/>
					</div>

					<div className="w-full bg-white flex flex-col space-y-2 p-3">
						<h3 className="font-bold text-gray-800 lg:text-2xl text-md">
							{data.model_name || "BMW E46"}
						</h3>
						<p className="lg:text-lg text-gray-500 text-base">
							{" "}
							{strTruncator(data.description, 200)}{" "}
						</p>
						<p className="text-xl font-black text-gray-800">
							<span className="text-[#ffe001] text-base font-semibold">
								{" "}
						...
						<div className="w-full flex flex-col xl:flex-row gap-3 lg:gap-4 xl:gap-0 xl:justify-between xl:items-center">
							<div className="w-full xl:w-[30%] flex flex-row gap-[10px] lg:gap-[20px] justify-center lg:justify-start items-center">
								<p> {data.model_year} </p>|<p> {data.mileage_km + " km"} </p>|
								...
								<>
									<div className="w-full xl:w-[70%] flex flex-col md:flex-row lg:flex-row justify-center items-center gap-1">
										<div className="w-full text-black font-medium rounded-[30px] flex lg:flex-row flex-col gap-3">
											{data.accepted_bid && (
												<button className="bg-black w-full text-white font-bold rounded-[30px] px-4 py-2">
													You have accept the bid of ${data.accepted_bid.amount}
												</button>
											)}
											{data.avail_lock && (
												<button className="bg-black w-full lg:w-[80%] text-white font-bold rounded-[30px] px-4 py-2">
										...
										{!data.avail_lock &&
											...
											(availabilityUpdated === false ? (
												...
												<button className="bg-[#2FB500] w-full lg:w-[70%] text-white font-bold rounded-[30px] px-4 py-2">
						...
						{/* Add Payment Message */}
						{isAccept === true &&
							(data?.accepted_bid?.payment ||
							data?.accepted_bid?.payment === true ? (
								<></>
							) : (
								<div className="text-red-500 lg:text-end text-center">
									Buyer Payment is not received, Buyer cannot see vehicle
									availability.
								</div>
							))}
					</div>
				...
				{seeBids && !isAccept && (
					<>
						{bidsDetails.map((bid, index) => (
							<div
								key={index}
								className="w-full h-auto flex flex-col lg:flex-row gap-5 py-6 px-10 border-t-2 border-gray-200"
							>
								...
								<div className="basis-2/5 flex justify-center items-center">
									<button
										...
										className="bg-[#ffe001] w-auto text-white font-bold rounded-[30px] px-6 py-2"
									>
										Accept Bid
									</button>
								</div>
				...
				<Modal open={openFirst} onClose={handleCloseFirst}>
					<Box sx={style}>
						<div className="flex flex-row justify-evenly items-center py-24">
							<div
								onClick={() => handleOpenSecond("pickup")}
							...
							<div
								onClick={() => handleOpenSecond("dropof")}
								className="flex flex-col justify-center items-center cursor-pointer"
							>
					...
					<Box sx={styleSecond}>
						<div className="w-full relative flex flex-col justify-center items-center gap-6 py-14 px-8">
							...
							<h1 className="font-bold text-xl">
								{" "}
								Vehicle Availability ({(availType && availType) || ""}){" "}
							</h1>
							<div className="w-full flex flex-col justify-start items-start gap-3 cursor-pointer">
								...
								<TextField
									name="address"
									value={AvailabilityForm.address}
									onChange={handleAvailabilityFormChange}
									id="address"
									type="text"
									fullWidth
				...
				<Modal open={openThird} onClose={handleCloseThird}>
					<Box sx={styleSecond}>
						<div className="w-full relative flex flex-col justify-center items-center gap-6 py-14 px-8">
							<div className="w-full absolute right-0 top-0 flex justify-end items-end pt-6 pr-6 cursor-pointer">
